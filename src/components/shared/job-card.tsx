          ...
          <CardContent className="p-8">
            <div className="flex flex-col lg:flex-row gap-8">
              ...
              <div className="lg:w-64 flex flex-col justify-between pt-4 lg:pt-0 border-l border-zinc-800/50 pl-0 lg:pl-8">
                <div 
                  className={`bg-zinc-950/50 rounded-2xl p-5 border border-zinc-800 ${variant !== "client" ? "cursor-pointer hover:bg-zinc-900/80 hover:border-zinc-700 transition-all group/client" : ""}`}
                  onClick={() => variant !== "client" && job.client?.email && setIsClientProfileOpen(true)}
                >
                  <p className="text-[9px] font-bold uppercase tracking-[0.2em] text-zinc-600 mb-4 flex justify-between items-center">
                    <span>{variant === "client" ? "Project Status" : "Client Identity"}</span>
                    {variant !== "client" && <ExternalLink className="w-2.5 h-2.5 opacity-0 group-hover/client:opacity-100 transition-opacity" />}
                  </p>
                  {variant === "client" ? (
                    ...
                    <div className="flex items-center gap-3">
                      <div className="w-10 h-10 rounded-full bg-gradient-to-br from-zinc-700 to-zinc-900 flex items-center justify-center text-white font-bold text-sm border border-zinc-700 group-hover/client:border-zinc-500 transition-colors">
                        {job.client?.fullName?.[0] || 'C'}
                      </div>
                      <div>
                        <p className="text-white font-bold text-sm truncate group-hover/client:text-blue-400 transition-colors">{job.client?.fullName || 'Verified Client'}</p>

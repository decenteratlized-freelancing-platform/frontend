...
import { Briefcase, Clock, BarChart, Layers, Tag, Calendar, User, Zap, ExternalLink } from "lucide-react"
            ...
            <Dialog open={isOpen} onOpenChange={onClose}>
                <DialogContent className="max-w-2xl max-h-[90vh] bg-zinc-950 border-zinc-800 text-zinc-100 flex flex-col p-0 gap-0 shadow-2xl overflow-hidden rounded-3xl">
                    ...
                    <div className="relative p-8 pb-6 border-b border-zinc-800 bg-zinc-900/20 shrink-0">
                        ...
                        <div 
                            className="flex items-center gap-2 text-zinc-400 text-sm font-medium cursor-pointer hover:text-white transition-colors group w-fit"
                            onClick={() => job.client?.email && setIsClientProfileOpen(true)}
                        >
                            <div className="w-5 h-5 rounded-full bg-zinc-800 border border-zinc-700 flex items-center justify-center text-[8px] font-bold group-hover:border-zinc-500">
                                {job.client?.fullName?.[0] || "C"}
                            </div>
                            <span>{job.client?.fullName || "Verified Client"}</span>
